...
ood_data = pickle.load(open('./data/VOC-Detection/' + args.model + '/'+args.name+'/random_seed' +'_'+str(args.seed)  +'/inference/openimages_ood_val/standard_nms/corruption_level_0/probabilistic_scoring_res_odd_'+str(args.thres)+'.pkl', 'rb'))
...
if args.energy:
    ...
    ood_score = -torch.logsumexp(torch.stack(ood_data['inter_feat'])[:, :-1], dim=1).cpu().data.numpy()
else:
