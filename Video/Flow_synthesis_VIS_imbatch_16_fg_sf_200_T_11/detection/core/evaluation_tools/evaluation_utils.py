...
def get_train_contiguous_id_to_test_thing_dataset_id_dict(
    ...
    if train_thing_dataset_id_to_contiguous_id == test_thing_dataset_id_to_contiguous_id:
        ...
        if 'coco_ood_val' == args.test_dataset and 'voc_custom_train' == cfg.DATASETS.TRAIN[0]:
            ...
            cat_mapping_dict = dict(
                ChainMap(*[{i: i + 1} for i in range(20)]))
        ####################  Start: this is Flow synthesis's custom addition to make it work on video datasets as well  ########################
        elif 'vis_coco_ood' == args.test_dataset and 'vis21_train' == cfg.DATASETS.TRAIN[0]:
            from collections import ChainMap
            cat_mapping_dict = dict(
                ChainMap(*[{i: i + 1} for i in range(40)]))
        elif 'nu_bdd_ood' == args.test_dataset and 'vis21_train' == cfg.DATASETS.TRAIN[0]:
            from collections import ChainMap
            cat_mapping_dict = dict(
                ChainMap(*[{i: i + 1} for i in range(40)]))
        elif 'coco_2017_val_ood_wrt_bdd' == args.test_dataset and 'bdd_tracking_2k_train' == cfg.DATASETS.TRAIN[0]:
            from collections import ChainMap
            cat_mapping_dict = dict(
                ChainMap(*[{i: i + 1} for i in range(8)]))
        elif 'nu_bdd_ood' == args.test_dataset and 'bdd_tracking_2k_train' == cfg.DATASETS.TRAIN[0]:
            from collections import ChainMap
            cat_mapping_dict = dict(
                ChainMap(*[{i: i + 1} for i in range(8)]))
        ####################  End: this is Flow synthesis's custom addition to make it work on video datasets as well  ########################  
        else:
            cat_mapping_dict = dict(
                (v, k) for k, v in test_thing_dataset_id_to_contiguous_id.items())
            ...
            #elif 'vis_coco_ood' in args.test_dataset and 'vis21_train' in cfg.DATASETS.TRAIN[0]:
                #dataset_mapping_dict = dict(
                    #(v, k) for k, v in metadata.VIS_TO_COCO_CONTIGUOUS_ID.items()) 
            else: 
                ValueError(
