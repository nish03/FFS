...
**Note:** The path needs to be created if you directly use the pre-trained models instead of first performing the training from scratch. Additionally, If you are using the weights from the pre-trained model, then please use the correct path for WEIGHTS in the config file as ```/path/to/Flow-Feature-Synthesis/detection/data/VOC-Detection/faster-rcnn/regnetx/random_seed_0/model_final.pth```. If you do not perform this step, there may a situation where detectron2 framework still uses initial weights and you may not get the desired results.
