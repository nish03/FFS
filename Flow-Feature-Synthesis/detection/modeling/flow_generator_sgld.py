...
class ROIHeadsLogisticGMMNew(ROIHeads):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.layer1 = Node(self.in1, GLOWCouplingBlock, {'subnet_constructor': self.subnet_fc, 'clamp': 2.0}, name=F'coupling_{0}')
    ...
    def _forward_box(self, features: Dict[str, torch.Tensor], proposals: List[Instances], iteration: int):
        ...
        #print("Box features before box_head has shape:", box_features.shape)
        box_features = self.box_head(box_features)
        #print("Box features after box_head has shape:", box_features.shape)
        ...
        if self.training:
            ...
            if len(proposals):
                ...
                if iteration < self.start_iter:
                    ...
                    # maintaining an ID data queue for each class.
                    z, sldj = self.flow_model(box_features[indices_numpy,].detach().cuda())
                    #compute the mean of negative log-likelihoods for the inlier samples
                    nll_loss = self.NLLLoss(z, sldj)
                    #compute the log-likelihoods for the inlier samples
                    ll_inliers = - self.NLL(z, sldj)
                    #randomly initialize flow's latent space
                    z_randn = torch.randn((self.sample_from,1024), dtype = torch.float32).cuda()     
                    #synthetize negative sample in flow's reverse direction
                    ood_samples, _ = self.flow_model(z_randn, rev=True)
                    #compute the jacobian determinant
                    _, sldj_neg = self.flow_model(ood_samples)
                    #compute the log likelihood of the negative sample
                    ll_neg = - self.NLLLoss(z_randn, sldj_neg)
                    ####################################################
                    #start langevin dynamics based sampling
                    ####################################################
                    count = 0
                    step_size = 0.5
                    while True:
                        if ll_neg < ll_inliers.min():
                            break
                        #compute the gradient of log likelihood w.r.t. negative sample
                        grad_ll_x = autograd.grad(ll_neg, ood_samples)[0]
                        #update the negative sample
                        ood_samples = ood_samples - step_size * grad_ll_x
                        #forward the updated negative sample through flow
                        z_new_neg, sldj_new_neg = self.flow_model(ood_samples)
                        #compute loglikelihood of new negative sample
                        ll_neg = - self.NLLLoss(z_new_neg, sldj_new_neg)
                        count += 1
                    #print(count)                    
                    ####################################################
                    #end of langevin dynamics based sampling
                    ####################################################
